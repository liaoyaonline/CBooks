...
int regexbook(char * inbookid)
{
    ...
    booklistsnum = BookInfo.AddListCount[0] - '0';//计算书本数目
    for(int i = 1;i < (int)strlen(BookInfo.AddListCount);i++)
    {
        booklistsnum = (booklistsnum * 10) + (BookInfo.AddListCount[i] - '0');
    }
    printf("%d\n",booklistsnum);
    if(booklistsnum > 0)
    {
...
int regexbookfile(char* booklistid,const char* pattern,char flag)
{
    ...
    while(regexec(&reg,p,nmatch,pmatch,0) != REG_NOMATCH)
    {
        ...
        case 'f' :
            printf("读取书本总评分\n" );
            strncpy(BookInfo.BookScore,tmp,len+1);
            break;
        case 'g':
            printf("读取书本评分人数\n");
            strncpy(BookInfo.BookScoreCount,tmp,len+1);
            break;
        case 'h':
            printf("读取评一星占比\n");
            strncpy(BookInfo.OneStarRate,tmp,len+1);
            break;
        case 'i':
            printf("读取二星占比\n");
            strncpy(BookInfo.TwoStarRate,tmp,len+1);
            break;
        case 'j':
            printf("读取评三星占比\n");
            strncpy(BookInfo.ThreeStarRate,tmp,len+1);
            break;
        case 'k':
            printf("读取评四星占比\n");
            strncpy(BookInfo.FourStarRate,tmp,len+1);
            break;
        case 'l':
            printf("读取评五星占比\n");
            strncpy(BookInfo.FiveStarRate,tmp,len+1);
            break;
        case 'm':
            printf("读取书本类型\n");
        ...
        case 'n':
            printf("读取添加这本书的书单总数\n");
            strncpy(BookInfo.AddListCount,tmp,len+1);
            break;
        case 'o':
            printf("读取书本的介绍\n");
...
int InitBooks()
{
    ...
    memset(BookInfo.BookScore,0,sizeof(BookInfo.BookScore));
    memset(BookInfo.BookScoreCount,0,sizeof(BookInfo.BookScoreCount));
    memset(BookInfo.OneStarRate,0,sizeof(BookInfo.OneStarRate));
    memset(BookInfo.TwoStarRate,0,sizeof(BookInfo.TwoStarRate));
    memset(BookInfo.ThreeStarRate,0,sizeof(BookInfo.ThreeStarRate));
    memset(BookInfo.FourStarRate,0,sizeof(BookInfo.FourStarRate));
    memset(BookInfo.FiveStarRate,0,sizeof(BookInfo.FiveStarRate));
    memset(BookInfo.BookClass,0,sizeof(BookInfo.BookClass));
    memset(BookInfo.AddListCount,0,sizeof(BookInfo.AddListCount));
    ...
    memset(BooklistInfo.BooklistCount,0,sizeof(BooklistInfo.BooklistCount));
    memset(BooklistInfo.BooklistKeep,0,sizeof(BooklistInfo.BooklistKeep));
